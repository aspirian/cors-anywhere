...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    targetWhiteList: [],            // Requests to any other site othen than present in this list would be blocked
  ...
  return function(req, res) {
    ...
    if (corsAnywhere.targetWhiteList.length && targetURL.length) {
       var result = false;
       for (let i = 0; i < corsAnywhere.targetWhiteList.length; i++) {
          var target = corsAnywhere.targetWhiteList[i];
	  if(targetURL.startsWith(target)){
	     result = true;
        }
    }
    if (!result) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('Sorry, the target "' + targetURL + '" is not whitelisted by this server');
      return;
    }  
}
